...
public partial class BerberRandevularSayfasi : ContentPage
{
    ...
    private async Task RandevulariYukleAsync()
    {
        try
        {
            var gelecekDto = await _api.GetBerberGelecekRandevularAsync(UserSession.KullaniciId);
            var gecmisDto = await _api.GetBerberGecmisRandevularAsync(UserSession.KullaniciId);
