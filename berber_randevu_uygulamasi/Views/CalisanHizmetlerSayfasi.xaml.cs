...
public partial class CalisanHizmetlerSayfasi : ContentPage
{
    ...
    public CalisanHizmetlerSayfasi(ApiClient api)
    {
        InitializeComponent();
        _api = api;
    }
    ...
    private async Task HizmetleriYukleAsync()
    {
        try
        {
            var response = await _api.GetCalisanHizmetleriAsync(UserSession.KullaniciId);
    ...
    async void AktifPasif_Clicked(object sender, EventArgs e)
    {
        ...
        try
        {
            var result = await _api.HizmetAktifPasifAsync(h.HizmetID);
