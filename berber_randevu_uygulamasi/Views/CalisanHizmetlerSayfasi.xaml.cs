...
public partial class CalisanHizmetlerSayfasi : ContentPage
{
    ...
    public CalisanHizmetlerSayfasi()
    {
        InitializeComponent();
    }
    ...
    private async Task HizmetleriYukleAsync()
    {
        ...
        {
            // ÇALIÞAN MANTIÐI:
            // 1) KullaniciId -> CalisanID
            // 2) KullaniciId -> (calisanlar üzerinden) BerberID
    ...
    // ÇALIÞANIN BAÐLI OLDUÐU BERBERÝ BULUR (Berberler tablosuna bakmaz)
    private async Task<int> GetBerberIdByCalisanKullaniciIdAsync(int kullaniciId)
    {
        await using var conn = new NpgsqlConnection(DbConfig.ConnectionString);
        await conn.OpenAsync();

        // Varsayým: calisanlar tablosunda "BerberID" kolonu var
        string sql = @"
            SELECT ""BerberID""
            FROM calisanlar
